...
class Block_Controller(object):
    ...
    def GetNextMove(self, nextMove, GameStatus):
        ...
        if self.CurrentShape_index==1:
            direction0 = 0
            x0 = random.choice([0,9])
            strategy = (direction0, x0, 1, 1)
            
        elif self.CurrentShape_index==2 or self.CurrentShape_index==3 or self.CurrentShape_index==4 or self.CurrentShape_index==5 or self.CurrentShape_index==6 or self.CurrentShape_index==7:
    ...
    def calcEvaluationValueSample(self, board):
        ...
        # nHoles
        for x in holeConfirm:     #listを繰り返している？？？
            nHoles += abs(x)      #各x座標の穴の数の合計
        ...
        for i in range(len(BlockMaxY) - 1):
            ...
            BlockMaxDy += [val]
        for x in BlockMaxDy:
            absDy += abs(x)         #隣の列との高さの差分を合計して凸凹具合を表現

        #### maxDy
        maxDy = max(BlockMaxY) - min(BlockMaxY)       #復活
        #### maxHeight
        maxHeight = max(BlockMaxY) - fullLines        #復活
        ...
        score = score - nHoles * 100.0               # try not to make hole 1 → 100
        score = score - nIsolatedBlocks * 1000.0      # try not to make isolated block 1 → 1000
        score = score - absDy * 50.0                # try to put block smoothly 1 → 50
